...
"""
MatgoAI. Judge the field, and decide what to put, what to get 
"""
# MatgoAI. Judge the field, and decide what to put, what to get 
class AI:
			...
			for k in range(12):
				if len(field[k])>0:
					if field[k][0].month==deck.check.month:
						...
						next.append(field[k])
			if next!=[]:
				if self.deck_and(next, "고도리", "초단") and len(aiobject.godori)==2 and len(aiobject.chodan)==2:
	...
	def determine(self, aiobject, playerobject, field, deck):
	...
	def go_stop(self, aiobject, playerobject, field, deck):
		if self.__ai=="easy":
			return False 
		if self.__ai=="normal":
							...
							if 4<=playerobject.score<=6:
								...
								for k in range(12):
									for i in range(len(playerobject.hand)):
										if len(field[k])>=1:
											if field[k][0].month==playerobject.hand[i].month:
												check2=False
								check3=True
								for k in range(len(playerobject.hand)):
									if deck.check.month==playerobject.hand[k].month:
										check3=False
								if length==0:
									if check3:
										return True
	...
	def cardchoose_condition(self, con):
		if con!="뻑":
			if con!="열끗" or con!="쌍피":
	...
	def cardchoose_condition2(self, con1, con2):
	...
	def cardchoose_bicondition(self, con):
		for k in range(len(self.__aiobject.hand)):
			for j in range(12):
				if len(self.__field[j])!=0:
					for i in range(len(self.__field[j])):
